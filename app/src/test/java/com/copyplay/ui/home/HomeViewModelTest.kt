...
class HomeViewModelTest {
    ...
    fun `probes known servers concurrently and marks availability`() = runTest {
        ...
        val viewModel = HomeViewModel(
            ...
            tailscaleDetector = FakeTailscaleDetector(TailscaleStatus.Connected),
            availabilityProber = prober,
        )
        advanceUntilIdle()

        val state = viewModel.state.value
        assertEquals(TailscaleStatus.Connected, state.tailscaleStatus)
...
private class FakeTailscaleDetector(
    ...
    override suspend fun detect(): TailscaleStatus = status
}
