...
class ServerConnectionRepository(
    ...
    suspend fun saveDiscovered(server: DiscoveredCopypartyServer): ServerConfig {
        val serverConfig = ServerConfig(server.baseUrl)
        serverConfigStore.save(serverConfig)
        serverConfigStore.rememberSuccessfulConnection(
            serverConfig = serverConfig,
            identity = CopypartyServerIdentity(server.displayName),
        )
        return serverConfig
    }
