...
- 2026-06-30: Confirmed local copyparty is running as `copyparty.service` on port `3923`, with anonymous read-only `/torrents` and `/videos` shares. Root and nested `?ls` structured listings work at `http://127.0.0.1:3923`, and the phone-facing Tailscale URL should be `http://100.90.244.72:3923`. Verified a real MP4 responds with `206 Partial Content`, `Accept-Ranges: bytes`, and `Content-Type: video/mp4` for `Range: bytes=0-0`. App-on-phone playback and interactive seek verification are still pending.
